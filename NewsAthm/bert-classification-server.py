...
reset_log('./logs/bert-classification-server_training.log')
logger = logging.getLogger(__name__)
logging.info('This is a log info')

# # 配置日志记录器
# logging.basicConfig(filename='./logs/bert-classification-server_training.log', level=logging.INFO, format='%(asctime)s - %(message)s', datefmt='%Y-%m-%d %H:%M:%S')
...
model_path = f"./models/{model_name}_classification_model_epoch_{num_epochs}.pth"

# 保存模型到文件
torch.save(model.state_dict(), model_path)

# # 保存模型到文件
# torch.save(model.state_dict(), 'bert_classification_model_e10.pth')
