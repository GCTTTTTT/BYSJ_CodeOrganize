# undersampled 欠采样平衡后数据进行训练 ephoch=10 再训练5轮  2遍
import pandas as pd
import torch
...
reset_log('./logs/bert-classification-server-undersampled-continue_training.log')
logger = logging.getLogger(__name__)
logging.info('This is a log info')

# # 配置日志记录器
# logging.basicConfig(filename='./logs/bert-classification-server-undersampled-continue_training.log', level=logging.INFO, format='%(asctime)s - %(message)s', datefmt='%Y-%m-%d %H:%M:%S') ## 
...
# 加载已有模型！！
# 加载之前保存的模型参数
model.load_state_dict(torch.load('bert_model_undersampled_e15.pth')) ## !!!!
...
# # 保存模型到文件
# torch.save(model.state_dict(), 'bert_model_undersampled_e20.pth')
