# undersampled 欠采样平衡后数据进行训练 ephoch=10  使用XL-NET-base-cased
# update 3.6 FIX版本
# update:3.9 FIX2
import pandas as pd
import torch
...
reset_log('./logs_FIX2/XL-NET-base-cased-classification-server-undersampled_training.log')  ### update
...
# file_path = './datasets_FIX/FIX_deduplicated_mangoNews_Nums3000p_CategoryMerge_new_undersampled.csv' # new  ### update
file_path = './datasets_FIX2/FIX2_deduplicated_mangoNews_Nums3000p_CategoryMerge_new_undersampled.csv' # new  ### update
...
# 加载Albert模型和分词器
# Bert_path = './AlBert-base'
# 加载roberta模型和分词器
# Bert_path = './roberta-base'
model_path = './XL-NET-base-cased'
# model_path = './XLNet_NEW/xlnet_cased_L-12_H-768_A-12' # NEW


# # 加载RoBERTa模型和分词器
# model_path = 'path/to/roberta_model'
...
# tokenizer = AlbertTokenizer.from_pretrained(Bert_path) # AlBert
...
# model = AlbertForSequenceClassification.from_pretrained(Bert_path, num_labels=num_classes)
# model = RobertaForSequenceClassification.from_pretrained(Bert_path)
model = XLNetForSequenceClassification.from_pretrained(model_path, num_labels=num_classes)
...
# # 加载之前保存的模型参数
# # 2.22 12:25 e10
# model.load_state_dict(torch.load('./models/XL-Net/XL-NET-base-cased_classification_undersampled_new_epoch_10.pth')) ## !!!!
# ======================================================================================================================================================
...
model_path = f"./models_FIX2/{model_name}_classification_undersampled_new_epoch_{num_epochs}.pth"  ##  ### update
...
# 这是一个使用XLNet进行分类任务的代码，任务描述根据语料csv中的body列内容（新闻正文）预测其新闻类别（category1）列，这是一个由bert模型训练修改所得的代码，在bert上分类结果还行，但使用XLNet后分类结果很差，但根据XLNet的描述，他应该比bert更适用于分类任务，请问是否我的代码中出现了任何问题导致XLNet的表现不佳？请帮我在代码基础上改进代码并给出解释，然后给出完整代码