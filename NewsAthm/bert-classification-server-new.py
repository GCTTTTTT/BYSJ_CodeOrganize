# 平衡前原数据训练 类别再合并（16-12）9.9g e=10
import pandas as pd
import torch
...
reset_log('./logs/bert-classification-server-new_training.log')
logger = logging.getLogger(__name__)
logging.info('This is a log info')

# # 配置日志记录器
# logging.basicConfig(filename='./logs/bert-classification-server-new_training.log', level=logging.INFO, format='%(asctime)s - %(message)s', datefmt='%Y-%m-%d %H:%M:%S')
...
file_path = './datasets/deduplicated_mangoNews_Nums3000p_CategoryMerge_new.csv'          # 去重后保留类别对应语料数在3000+并合并同义类别的数据集（9.9g) 类别再合并 16-12
...
model_path = f"./models/{model_name}_classification_model_new_epoch_{num_epochs}.pth"

# 保存模型到文件
torch.save(model.state_dict(), model_path)

# # 保存模型到文件
# torch.save(model.state_dict(), 'bert_classification_model_new_e10.pth')
